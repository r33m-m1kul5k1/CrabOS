...
macro_rules! wrmsr {
    ($msr:expr, $value:expr) => {
        unsafe { core::arch::asm!("wrmsr", in("edx") $value >> 32, in("eax") $value & 0xFFFFFFFF, in("ecx") $msr) };
    };
}
...
pub fn rdmsr(msr: u64) -> u64 {
    ...
    unsafe { core::arch::asm!("rdmsr", out("edx") value_high, out("eax") value_low, in("ecx") msr) };
