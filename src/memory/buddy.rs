...
impl Buddy {
    ...
    fn get_order(&self, size: usize) -> Option<usize> {
        ...
        let max_size = self.region.get_region_byte_size() as usize; 
        if size > max_size {
            return None
        }
        ...
        let request_order = cmp::min(next_order - 1, self.region.get_region_byte_size() as usize);
        Some(request_order)
    ...
    pub fn allocate(&mut self, size: usize, alignment: usize) -> Option<PhysAddr> {
        ...
        self.get_order(size).and_then(|request_order| {
            ...
            .map(|block| {
                ...
                let offset = block as u64 * (self.region.get_region_byte_size() >> request_order as usize) as u64;
